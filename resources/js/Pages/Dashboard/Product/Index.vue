<script setup>
    ...
    watch(params, (newParams, oldParams) => {
        ...
        Inertia.get(route('dashboard.product.index'), params, {
            replace: true,
            preserveState: true
        })
    }, {
    ...
    <AppLayout title="Product">
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Product List</h2>
                <InertiaLink :href="route('dashboard.product.create')" class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-white uppercase transition bg-gray-800 border border-transparent rounded-md hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25">Create Product</InertiaLink>
            </div>
        </template>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Creator</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr v-for="row in products.data" :key="row.id">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <a :href="row.photo" target="_blank" class="flex-shrink-0 w-10 h-10">
                                                <img class="object-cover w-10 h-10 rounded-full" :src="row.photo" />
                                            </a>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <a :href="row.creator?.profile_photo_url" target="_blank" class="flex-shrink-0 w-10 h-10">
                                                <img class="object-cover w-10 h-10 rounded-full" :src="row.creator?.profile_photo_url" :alt="row.creator?.name" />
                                            </a>
                                            <div class="ml-4">
                                                <div class="overflow-hidden text-sm font-medium text-gray-900 w-44 overflow-ellipsis">
                                                    {{ row.creator?.name }}
